    ...
    while True:
        try:
            ...
            if (msg[0] == "@"):
                ...
                decrypted_key = decryptor.decrypt(
                    binascii.unhexlify(bytes(parsed_msg[2], "utf-8")))
                print("Randomly generated key from the sender : ",
                      decrypted_key.decode("utf-8"))
...
while True:
    ...
    if (len(parsed_msg) > 1):
        ...
        cipher = aes.encrypt(
            parsed_msg[1]) + bytes(":", "utf-8") + binascii.hexlify(encrypted_key)
